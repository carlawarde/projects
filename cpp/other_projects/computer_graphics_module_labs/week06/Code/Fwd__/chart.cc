...
void lineChart (void)
{
  ...
  glBegin(GL_LINE_STRIP);
    for (int k = 0; k < 12; k++)
      glVertex2i(x + k*(winWidth/12), dataValue[k]);
  glEnd();

  glColor3f(1.0, 0.0, 0.0);	// Set marker colour to red
  for (int k = 0; k < 12; k++)
  {
    glRasterPos2i(xRaster + k*(winWidth/12), dataValue[k]-4);
...
void keyPress(unsigned char key, int x, int y) 
{
  ...
  else if(key =='p' || key=='P') chart = 0;
  else if(key =='b' || key=='B') chart = 1;
  else if(key =='l' || key=='L') chart = 2;
  else if(key == 'z')
    {
      zoom = zoom * 0.75;
      glLoadIdentity ( );
...
{
	switch (option) {
		case PIE : chart = 0; break;
		case BAR : chart = 1; break;
	        case LINE : chart = 2; break;
